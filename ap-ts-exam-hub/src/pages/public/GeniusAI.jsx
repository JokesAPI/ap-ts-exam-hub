...
export default function GeniusAI() {
  ...
  const startListen = useCallback(() => {
    ...
    rec.lang           = 'en-IN' // Chrome accepts Telugu/Hindi/English with en-IN
    ...
    rec.onresult = (e) => {
      ...
      setInput(transcript)
      setTimeout(() => doSend(transcript, lang), 100)
    }
...
Based on these answers, give a detailed personalized weakness analysis.`
  ...
  async function doSend(textArg, langArg) {
    ...
    try {
      ...
      const clean = stripMd(reply)
      // Detect language directly from AI reply text
      const replyLang = detectScript(clean)
      // Update state for future messages
      if (replyLang !== 'en-US') setDetectedLang(replyLang)
      // Use replyLang directly -- do NOT rely on stale state
      const speakLang = replyLang !== 'en-US' ? replyLang : (finalLang || 'en-US')
      addMsg(activeTab, [...withUser, { role: 'assistant', content: clean }])
      // Speak in the language the AI actually replied in
      if (voiceOn) speak(clean, speakLang)
    } catch (err) {
      console.error(err)
