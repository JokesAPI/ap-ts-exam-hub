...
function safeDate(dateStr) {
  ...
  return isNaN(d) ? 'TBA' : d.toLocaleDateString('en-IN')
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // ── Fix: proper error handling for both queries ───────────────────────────
    supabase
      .from('notifications')
      .select('*')
      ...
      .then(({ data, error }) => {
        if (!mounted) return
        if (error) { console.error('Notifications error:', error); setNotifError(true); return }
        setNotifications(data || [])
      })
      .catch(err => { if (mounted) { console.error(err); setNotifError(true) } })
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (!mounted) return
        if (error) { console.error('Exams error:', error); setExamsError(true); return }
        setExams(data || [])
      })
    ...
    <Layout>
      ...
      <section className="bg-gradient-to-br from-primary-800 via-primary-700 to-primary-600 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-blue-200 mb-8">
            APPSC, TSPSC, DSC, TET, AP Police -- All Exams One Place
          </p>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
        <div className="card p-8">
          ...
          <h3 className="text-lg font-bold mb-3 text-gray-800 dark:text-gray-100">Exams We Cover</h3>
          ...
          <h3 className="text-lg font-bold mb-3 text-gray-800 dark:text-gray-100">Why Students Choose AP TS Exam Hub</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="card p-6">
            ...
            {notifError ? (
              <p className="text-red-400 text-sm">Could not load notifications. Please refresh.</p>
              ...
              <ul className="space-y-3">
                {notifications.map(n => (
                  <li key={n.id} className="border-l-2 border-primary-400 pl-3">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{safeDate(n.created_at)}</p>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="card p-6">
            ...
            {examsError ? (
              <p className="text-red-400 text-sm">Could not load exams. Please refresh.</p>
            ) : exams.length === 0 ? (
