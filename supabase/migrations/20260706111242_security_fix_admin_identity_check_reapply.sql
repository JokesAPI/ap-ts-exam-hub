create or replace function public.approve_draft(p_draft_id uuid, p_admin_id uuid, p_notes text DEFAULT NULL::text)
 returns void
 language plpgsql
 security definer
as $function$
begin
  ...
  where id = p_draft_id and status = 'validated';
  if not found then
    raise exception 'Draft not found or not in validated status.';
  end if;
...
grant execute on function approve_draft(uuid, uuid, text) to authenticated;
grant execute on function reject_draft(uuid, uuid, text) to authenticated;
grant execute on function publish_draft(uuid, uuid) to authenticated;
grant execute on function validate_draft(uuid) to authenticated, service_role;
grant execute on function check_duplicate_draft(text, text, text, text) to authenticated, service_role;