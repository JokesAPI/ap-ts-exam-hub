-- ROLLBACK for 20260721045809_phase6_5a3_pre_require_valid_test_id_on_question_drafts
--
-- Restores validate_draft() and publish_draft() to their pre-fix definitions,
-- reconstructed verbatim from pg_get_functiondef() captured immediately before
-- the migration was applied.
--
-- WARNING: reverting reinstates the behaviour that question drafts need no
-- test_id, and that publish_draft() substitutes the exam_slug or the literal
-- 'general' -- neither of which exists in mock_tests. Both produce orphaned
-- questions silently.
--
-- IMPORTANT ORDERING: 20260721090951 (the mock_questions.test_id foreign key)
-- must be rolled back FIRST. With that FK in place, the restored fallback chain
-- raises 23503 at publish time instead of writing an orphan -- a louder failure
-- than the one this rollback is trying to restore.
--
-- CREATE OR REPLACE preserves grants, ownership and SECURITY DEFINER settings.
-- No data is touched.
